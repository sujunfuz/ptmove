<template>
	<view class="wrapper" @touchmove.stop.prevent="()=>{return false}">
		<view class="w-top flex">
			<text>{{addressData.userName}}</text>
			<text class="top-txt">{{addressData.mobile}}</text>
		</view>
		<view class="w-mid u-line-2">{{addrDes}}</view>
		<view class="flex-cb marT20">
			<view class="flex-c">
				<u-checkbox-group v-model="checked" @change="changeSelect" >
					<u-checkbox  
						:disabled="addressData.defaultIs=='YES'"
						 activeColor="#E1251B" labelColor="#9C9E9C"
						:label="$t('common.moren')" name="YES">
					</u-checkbox>
				</u-checkbox-group>
			</view>
			<view class="flex-c">
				<view class="allbtn btn1" @click.stop="goToPage('/pages/mine/addaddress?openType=edit&&id='+addressData.id)">
					{{$t('common.edit')}}</view>
				<view class="allbtn btn2" @click.stop="deleteAddress">{{$t('common.del')}}</view>
			</view>
		</view>
	</view>
</template>
